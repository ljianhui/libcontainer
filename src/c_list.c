...
static c_list_node* c_list_node_create(const c_list *lst, const void *elem)
{
	c_list_node *nd = (c_list_node*)malloc(sizeof(c_list_node) + lst->elem_size);
	if (nd == NULL)
	{
		return NULL;
	}
	...
	lst->opt.dump(nd->elem, elem, lst->elem_size);
...
static c_list_node* c_list_get_node(const c_list *lst, int index)
{
	int i = 0;
...
{
	if (dump == NULL || release == NULL || compare == NULL || elem_size == 0)
	{
		return NULL;
	}
	...
	lst->size = 0;
	lst->opt.dump = dump;
	lst->opt.release = release;
	lst->opt.compare = compare;
...
c_list* c_list_clone(const c_list *src)
{
	...
	c_list *dst = c_list_create(src->opt.dump, src->opt.release, src->opt.compare, src->elem_size);
	if (dst == NULL)
	{
		return NULL;
	}
	...
	{
		dst_node = c_list_node_create(src, src_node->elem);
		if (dst_node == NULL)
		{
			c_list_destroy(dst);
			return NULL;
...
void c_list_clear(c_list *lst)
{
	...
	{
		lst->opt.release(free_node->elem);
		free(free_node);
		free_node = next_node;
...
void* c_list_add_first(c_list *lst, const void *elem)
{
	...
	c_list_node *nd = c_list_node_create(lst, elem);
	if (nd == NULL)
	{
		return NULL;
	}
...
void* c_list_add(c_list *lst, const void *elem, int index)
{
	...
	c_list_node *pos = c_list_get_node(lst, index);
...
int c_list_remove_elem(c_list *lst, const void *elem)
{
	...
	while (nd != lst->head)
	{
		if (lst->opt.compare(nd->elem, elem) == 0)
		{
...
int c_list_remove(c_list *lst, int index)
{
	...
	c_list_node *nd = c_list_get_node(lst, index);
	c_list_unlink(lst, nd);
	return 1;
}
...
void c_list_set_first(c_list *lst, const void *elem)
{
	...
	lst->opt.release(lst->head->next->elem);
	lst->opt.dump(lst->head->next->elem, elem, lst->elem_size);
}
...
void c_list_set_last(c_list *lst, const void *elem)
{
	...
	lst->opt.release(lst->head->prev->elem);
	lst->opt.dump(lst->head->prev->elem, elem, lst->elem_size);
}
...
void c_list_iter_add(c_list_iter *it, const void *elem)
{
	...
	c_list_node *nd = c_list_node_create(it->lst, elem);
	if (nd == NULL)
	{
		return;
	}
